<x-layout>
    <section class="cont">
        ...
        <!--Comprueba si el usuario logueado y el que hizo la solicitud son el mismo y muestra los anuncios del usuario logueado-->
        @if ($id == $usuarioLog)
            @if ($listadoAnuncios->isEmpty())<!--Si el usuario no ha creado ningun anuncio muestra un mensaje-->
            ...
            @else <!--Listado de anuncios de usuario-->
                            ...
                            <header class="cont__anuncios__anuncio__header">
                                @foreach ($habilidades as $habilidad)
                                    @if ($anuncio->habilidad_ofrecida == $habilidad->id)
                                        <h4 class="cont__anuncios__anuncio__header__titulo">{{ $habilidad->titulo }}</h4>
                                    @endif
                                @endforeach
                                <article class="cont__anuncios__anuncio__header__datos">
                                    ...
                                    <article class="cont__anuncios__anuncio__header__datos__menu">
                                        ...
                                        <nav class="cont__anuncios__anuncio__header__datos__menu__contenido">
                                            <!--Menu desplegable para borrar o editar anuncio-->
                                            <form action="{{ route('anuncios.delete', $anuncio->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <label
                                                    class="cont__anuncios__anuncio__header__datos__menu__contenido__enlace"><i class="fa-solid fa-trash"></i> Eliminar
                                            ...
                                            <form action="/anuncio" method="get">
                                                ...
                                                @csrf
                                                <label
                                                    class="cont__anuncios__anuncio__header__datos__menu__contenido__enlace"><i class="fa-regular fa-pen-to-square"></i> Editar
            ...
            @endif
        @else<!--Listado de anuncios de usuario externo-->
                ...
                <article class="cont__anuncios__anuncio">
                    <form action="ver" method="post">
                        ...
                        <button type="submit" class="cont__anuncios__anuncio">
                            ...
                            <section class="cont__anuncios__anuncio__contenido">
                                <article class="cont__anuncios__anuncio__contenido__article">
                                    <h3 class="cont__anuncios__anuncio__contenido__article__titulo"> {{ $anuncio->titulo_of }}</h3>
                                    <p class="cont__anuncios__anuncio__contenido__article__descrip"> {{ $anuncio->descripcion_of }}</p>
                                </article>
                                <article class="cont__anuncios__anuncio__contenido__article">
                                    @foreach ($habilidades as $habilidad)
                                        @if ($anuncio->habilidad_buscada == $habilidad->id)
                                            <h3 class="cont__anuncios__anuncio__contenido__article__titulo">  {{ $habilidad->titulo }}</h3>
                                        @endif
                                    @endforeach
                                    <p class="cont__anuncios__anuncio__contenido__article__descrip"> {{ $anuncio->descripcion_Bus }}</p>
                                </article>
                            </section>
                            </button>
                        </form>
